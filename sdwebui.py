...
class SDWebUIExtension(NewelleExtension):
    ...
    def get_additional_prompts(self) -> list:
        return [
            PromptDescription("generate-image-sdwebui", "Generate Image", "Generate images using StableDiffusion WebUI", "- To generate images use: \n```generateimage\nprompt\n```\nUse detailed prompts, with english words separated by commas"),
        ]

    def get_extra_settings(self) -> list:
        return [
            ExtraSettings.EntrySetting("url", "URL", "URL of the StableDiffusion WebUI", "http://localhost:7860"),
            ...
            ExtraSettings.ScaleSetting("width", "Width", "Width of the generated image", 512, 128, 2048, 0),
            ExtraSettings.ScaleSetting("height", "Height", "Height of the generated image", 512, 128, 2048, 0),
            ExtraSettings.NestedSetting("advanced_settings", "Advanced Settings", "Advanced settings like steps, guidance scale...", [
                ExtraSettings.ScaleSetting("steps", "Steps", "Number of steps for the generation", 20, 1, 150, 0),
                ExtraSettings.ScaleSetting("guidance_scale", "CFG Guidance Scale", "Guidance scale for the generation", 7, 1, 20, 0),
                ExtraSettings.EntrySetting("seed", "Seed", "Seed for the generation", "-1"),
                ExtraSettings.MultilineEntrySetting("extra_payload", "Extra Options", "Manually specify extra options in JSON format", "{}", website="https://github.com/AUTOMATIC1111/stable-diffusion-webui/wiki/API")
            ])
        ]
    ...
    def generate_image(self, prompt: str, widget, msg_uuid):
        """Generate image using StableDiffusion WebUI API"""
        def show_error_icon():
        ...
        try:
            # Process the prompt using the template
            processed_prompt = self.get_setting("positive-prompt").replace("[input]", prompt)
            
            # Build API payload from settings
            payload = {
                "prompt": processed_prompt,
                "negative_prompt": self.get_setting("negative-prompt"),
                "width": int(self.get_setting("width")),
                "height": int(self.get_setting("height")),
                "steps": int(self.get_setting("steps")),
                "cfg_scale": float(self.get_setting("guidance_scale")),
                "n_iter": 1,
                "batch_size": 1,
            }
            
            # Handle seed setting
            seed_setting = self.get_setting("seed")
            if seed_setting and seed_setting != "-1":
                try:
                    payload["seed"] = int(seed_setting)
                except ValueError:
                    pass  # Use random seed if conversion fails
            
            # Parse and merge extra payload options
            try:
                extra_payload = self.get_setting("extra_payload")
                if extra_payload and extra_payload.strip():
                    extra_options = json.loads(extra_payload)
                    payload.update(extra_options)
            except (json.JSONDecodeError, TypeError):
                print("Warning: Invalid JSON in extra_payload setting, ignoring")
            
            # Make API call
            webui_url = self.get_setting("url")
            ...
            if 'images' in result and len(result['images']) > 0:
                ...
                image_path = os.path.join(self.cache_dir, f"{msg_uuid}.png")
                ...
                GLib.idle_add(widget.set_image_from_path, image_path, on_image_loaded)
            else:
                # No images in response
                GLib.idle_add(show_error_icon)
